...
function harmony_theme_customizer_css($args) {
	...
	$brndclr   = '#efefef';
	$excclrd   = '#e9e9e9';
	$fntfamily = ( empty( get_theme_mod( 'harmony_fontfamily' ) ) ) ? esc_attr( $fntfamily )  
			     : wp_strip_all_tags( get_theme_mod( 'harmony_fontfamily' ) );
	$fntalign  = ( empty( get_theme_mod( 'harmony_fontalign' ) ) ) ? esc_attr( $fntalign )  
			     : wp_strip_all_tags( get_theme_mod( 'harmony_fontalign' ) );
	$maxw      = ( empty( get_theme_mod( 'harmony_maxwidth' ) ) ) ? esc_attr( $maxw )
                 : get_theme_mod( 'harmony_maxwidth' );
	$brclr     = ( empty( get_theme_mod( 'harmony_brandcolor' ) ) ) ? esc_attr( $brndclr )
                 : get_theme_mod( 'harmony_brandcolor' );
	$excclr     = ( empty( get_theme_mod( 'harmony_shadowclr' ) ) ) ? esc_attr( $excclrd )
	...
	.page .single-column p, .single single-column p{ text-align: ' . esc_attr( $fntalign ) . ';}
	.site-branding, .harmony-small-sidebar, #nav a, .post-navigation a, .harmony-btt button{background: ' . esc_attr( $brclr ) .'; box-shadow: 0 0 0 1px ' . esc_attr( $excclr ) . ';}
	@media screen and ( min-width: 768px ) { .blog .single-column, .page .single-column, .single single-column{max-width: '. esc_attr( $maxw ) .'px;margin: 0 auto;}}';
